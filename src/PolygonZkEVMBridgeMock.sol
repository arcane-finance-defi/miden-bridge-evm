...
import "@zkevm/lib/TokenWrapped.sol";
import "@zkevm/lib/EmergencyManager.sol";
import "./interfaces/IPolygonZkEVMBridgeMock.sol";

/**
 * PolygonZkEVMBridge that will be deployed on Ethereum and all Polygon rollups
 * Contract responsible to manage the token interactions with other networks
 */
contract PolygonZkEVMBridgeMock is EmergencyManager, ReentrancyGuardUpgradeable, AccessControlUpgradeable, IPolygonZkEVMBridgeMock
{
    ...
    function initialize(
        ...
        _grantRole(CLAIMER_ROLE, bridgeAddress);
    }
    ...
    /**
     * @notice Verify sender role and withdraw tokens/ether
     * @param originNetwork Origin network
     * @param originTokenAddress  Origin token address, 0 address is reserved for ether
     * @param destinationNetwork Network destination
     * @param destinationAddress Address destination
    ...
    function claimAsset(
        ...
        uint256 amount,
        bytes calldata metadata
    ) external ifNotEmergencyState onlyRole(CLAIMER_ROLE) {
